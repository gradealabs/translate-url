...
function lastIndexOfPattern(str, regex) {
    // Clone the regex
    var parts = regex.toString().split('/');
    var flags = (parts.pop() + 'g').replace(/g{2,}/, 'g');
 ...
 */
function translateUrlByPathname(url, fromLang, toLang) {
    // Make a copy of the URL instance
    var URLCtor = url.constructor;
    var urlCopy = new URLCtor(url.toString());
    var pattern = new RegExp("([-|/])" + fromLang + "\\b");
    var k = lastIndexOfPattern(urlCopy.pathname, pattern);
    if (k >= 0) {
...
exports.translateUrlByPathname = translateUrlByPathname;
/**
 * Will tranform the given URL into a URL that has the fromLang language tag
 ...
 */
function translateUrlByHostname(url, fromLang, toLang, _a) {
    var _b = (_a === void 0 ? {} : _a).rootHostname, rootHostname = _b === void 0 ? '' : _b;
    // Make a copy of the URL instance
    var URLCtor = url.constructor;
    var urlCopy = new URLCtor(url.toString());
    ...
    if (urlCopy.hostname.startsWith(fromLang + ".")) {
        ...
        var translatedUrl = url.toString().replace(new RegExp("([-|.])" + fromLang + "\\b"), "$1" + toLang);
        ...
        if (translatedUrl === url.toString()) {
            var hostParts = urlCopy.hostname.split('.');
...
exports.translateUrlByHostname = translateUrlByHostname;
